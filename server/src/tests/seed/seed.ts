...
import Counter, { ICounterDocument } from "./../../models/counter";
...
// linkEntryCount ID does not exist the first time it tries to update,
// but the upsert option ensures it creates one the first time a new ID is used
export const resetCounter = (done: jest.DoneCallback) => {
  Counter.findByIdAndUpdate(
    "linkEntryCount",
    { count: 0 },
    { new: true, upsert: true },
    (err, counter: ICounterDocument) => {
      if (err) {
        throw new Error(err);
      }
      if (counter.count !== 0) {
        throw new Error("Error updating counter value to 0.");
      }
      return done();
    }
  );
